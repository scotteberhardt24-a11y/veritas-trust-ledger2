...
export default function ForgotUsernamePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(API + '/api/auth/forgot-username', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) });
    ...
    <div style={{ minHeight: '100vh', background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 24, marginBottom: 4 }}>Forgot Username</h1>
          <p style={{ color: 'var(--muted)', fontSize: 13 }}>Enter your email and we will send your account details.</p>
        </div>
        <div className="card" style={{ padding: '2rem' }}>
          {!sent ? (
            <form onSubmit={handleSubmit}>
              ...
              <button className="btn btn-gold" type="submit" disabled={loading} style={{ width: '100%', padding: '12px 0', fontSize: 14 }}>{loading ? 'Sending...' : 'Send Account Details'}</button>
            </form>
          ) : (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📧</div>
              <h3 style={{ fontSize: 18, marginBottom: 8 }}>Check Your Email</h3>
              <p style={{ color: 'var(--muted)', fontSize: 13 }}>If that email exists, your account details have been sent.</p>
            </div>
          )}
